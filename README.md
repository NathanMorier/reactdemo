...
!!!!! Conditional Loading Message !!!!! (conditional-loading-message branch)
...
!!!!! Handling Fetch Errors !!!!! (handling-fetch-errors branch)

This sets all sorts of conditions, including the addition or removal of error
messages, the 'loading' state, and of course the end results of a successful fetch.
...
!!!!! Making a Custom Hook !!!!! (making-a-custom-hook branch)

Remember all of that error message and fetch stuff? What if we needed to re-use that?
That's what this lesson is all about.

Create useFetch.js

Copy the constants and useEffect from Home.js, and add a few tweaks and name changes:
// useFetch.js
import {useState, useEffect} from 'react';
...
Then make the following changes to Home.js:
//Home.js
